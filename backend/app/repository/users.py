...
from app.core.dto import UserCreate, UserOut, UserShortInfo
...
def get_filtered_users(db: Session, username_substring: str, limit: int) -> list[UserShortInfo] | None:
    ilike_query = f"%{username_substring}%"
    matches = db.query(User).filter(User.username.ilike(ilike_query)).limit(limit).all()
...
def create_user(db: Session, new_user: UserCreate) -> UserOut:
    u = User(email=new_user.email, username=new_user.username, password_hash=hash_password(new_user.password))
    db.add(u)
    db.commit()
    db.refresh(u)
